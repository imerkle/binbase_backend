defmodule Exchange.Balance do
  ...
  def insert_balance(user_id, ticker, amount) do
    coin_id = Exchange.Utils.coin_id(ticker)

    %Exchange.Balance{}
    |> changeset(%{user_id: user_id, coin_id: coin_id, amount: amount})
    |> Exchange.Repo.insert()
  ...
  def update_all(balances) do
    ...
    |> Enum.reduce(Ecto.Multi.new(), fn ({balance, index}, multi) ->
        ...
        Ecto.Multi.run(multi, index, fn repo, _ ->
          {update_count, _} = from(b in Exchange.Balance, update: [inc: [amount_locked: ^balance.amount, amount: ^-balance.amount]], where: b.user_id == ^balance.user_id and b.coin_id == ^balance.coin_id)
          |> repo.update_all([])
          {:ok, update_count}
        end)
    end)
