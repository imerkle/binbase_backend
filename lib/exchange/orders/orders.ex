defmodule Exchange.Orders do

  import Ecto.Query
  alias Exchange.Order
  alias Exchange.Utils
  alias Exchange.Engine
  alias Exchange.Rabbit
  alias Exchange.Trigger
  ...
  def create_order(maker_id, token_rel, token_base, side, price, amount, options \\ []) do
    ...
    |> Ecto.Multi.run(:balance, fn repo, %{order: order, trigger: trigger} ->
      if trigger == nil do
        {coin_id, amount} =
        if side == false do
          {Utils.coin_id(token_base), order.amount * order.price}
        else
          {Utils.coin_id(token_rel), order.amount}
        end

        {update_count, _} = from(b in Exchange.Balance, update: [inc: [amount_locked: ^amount, amount: ^-amount]], where: b.user_id == ^order.maker_id and b.coin_id == ^coin_id)
        |> repo.update_all([])
